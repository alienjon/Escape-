...
EnvironmentData& EnvironmentData::operator=(const EnvironmentData& eData)
{
    // Check to make sure we aren't doing stuff with the same object.
    if(&eData != this)
    {
...
void EnvironmentData::deathOccurred(Entity& entity)
{
// @todo what happens when entities die?
}
...
std::string EnvironmentData::extract() const
{
	Logger::warn("EnvironmentData::extract() -> Not yet implemented");
	return "";//@todo implement saving the game
}
...
void EnvironmentData::handleInput(const Input& input)
{
